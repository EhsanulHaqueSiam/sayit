...
**Timing & easing**
- Default easing: `cubic-bezier(0.22, 1, 0.36, 1)` (ease-out-quart) for most UI. Use ease-out-quint `cubic-bezier(0.22, 1, 0.36, 1)` or ease-out-expo `cubic-bezier(0.16, 1, 0.3, 1)` for hero moments (page load, dictation start).
